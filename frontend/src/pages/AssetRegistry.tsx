...
import { Filter, Search, FileText, Trash2 } from "lucide-react";
...
export default function AssetRegistry() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['assets'],
    queryFn: () => apiClient.get('/assets').then(res => res.data)
  });
              ...
              <motion.tbody variants={container} initial="hidden" animate="show" className="font-bold">
                {assets.map((asset, i) => (
                  <motion.tr variants={item} whileHover={{ scale: 1.01, backgroundColor: "#f3f4f6" }} key={i} className="border-b-4 border-black transition-colors relative z-10 cursor-pointer bg-white">
                    <td className="p-6 font-black italic">{asset.tagId || asset.id}</td>
                    <td className="p-6 uppercase">{asset.name}</td>
                    <td className="p-6 uppercase">{asset.category?.name || "Uncategorized"}</td>
                    <td className="p-6">
                      <NeoBadge color={
                    ...
                    <td className="p-6 text-right flex justify-end gap-2">
                      ...
                      <NeoButton variant="black" className="px-4 py-2 text-xs inline-flex" onClick={(e) => { e.stopPropagation(); deleteMutation.mutate(asset.id); }}><Trash2 size={16} /></NeoButton>
                    </td>
                  </motion.tr>
                ))}
              </motion.tbody>
