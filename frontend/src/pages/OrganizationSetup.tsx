            ...
            <tbody className="font-bold">
              {departments.map((dept, i) => (
                <motion.tr whileHover={{ backgroundColor: "#f3f4f6" }} key={i} className="border-b-4 border-black transition-colors cursor-pointer">
                  ...
                  <td className="p-4 text-neutral-500 uppercase">{dept.parent?.name || "None"}</td>
                  <td className="p-4 uppercase">{dept.head?.name || "Unknown"}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-right flex justify-end gap-2">
                    ...
                    <NeoButton variant="black" className="p-2" onClick={() => deleteMutation.mutate(dept.id)}><Trash2 size={16} /></NeoButton>
                  </td>
                </motion.tr>
              ))}
            </tbody>
            ...
            <tbody className="font-bold">
              {categories.map((cat, i) => (
                <motion.tr whileHover={{ backgroundColor: "#f3f4f6" }} key={i} className="border-b-4 border-black transition-colors cursor-pointer">
                  ...
                  <td className="p-4 text-right flex justify-end gap-2">
                    <NeoButton variant="white" className="p-2"><Edit3 size={16} /></NeoButton>
                    <NeoButton variant="black" className="p-2" onClick={() => deleteMutation.mutate(cat.id)}><Trash2 size={16} /></NeoButton>
                  </td>
                </motion.tr>
              ))}
            </tbody>
...
function EmployeesTab() {
  ...
  const updateRoleMutation = useMutation({
    mutationFn: ({ id, role }: { id: number, role: string }) => apiClient.put(`/employees/${id}/role`, { role }),
